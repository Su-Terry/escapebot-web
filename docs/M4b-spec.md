...
>
> (「開新一局」已隨 M4a 完成——分享頁已有「挑戰 EscapeBot」CTA、`/play` 已有「開始新場景」——不在此重做。)（註:CTA 現文字「EscapeBot」是對外顯示名,待統一為 Cat Got Your Words —— code/UI 改動,不在文件範圍。下方 B1 同。）
...
- **來源(5/27)**:M4a 刻意先用簡單版(乾淨字卡:主題框 + 金句大字 + `EscapeBot · N turns` 署名);M3 的視覺(貓 / 場景 / pixel-art)定了之後,在 M4b 美化分享卡、跟 M3 對齊。（註:署名現文字「EscapeBot」是對外顯示名,待統一為 Cat Got Your Words —— code/UI 改動,不在文件範圍。）
